...
export const Header: React.FC<HeaderProps> = ({ onOpenDemo, onOpenStaffLogin }) => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-[#E7E7E2] transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-[72px] sm:h-20 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          <button
            ...
            className="min-h-11 px-4 py-2.5 text-xs font-semibold text-[#252525] border border-[#E7E7E2] rounded-xl hover:bg-[#FAF7F2] hover:border-[#252525] transition-all flex items-center gap-1.5 cursor-pointer"
          >
          ...
          <button
            onClick={onOpenDemo}
            className="min-h-11 px-5 py-2.5 text-xs font-bold text-[#121212] bg-[#18F28D] rounded-xl hover:bg-[#11dc7d] hover:shadow-md transition-all cursor-pointer"
          >
            Demo Al
          </button>
        </div>
        ...
        <button
          ...
          className="md:hidden grid h-11 w-11 place-items-center rounded-xl border border-[#E7E7E2] text-[#252525] hover:bg-[#FAF7F2] transition-colors"
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div id="mobile-navigation" className="md:hidden bg-white border-b border-[#E7E7E2] px-4 py-5 space-y-4 shadow-lg animate-fade-in">
          <nav className="flex flex-col space-y-2" aria-label="Mobil navigasyon">
            {navLinks.map((link) => (
              <a
                ...
                className="flex min-h-11 items-center justify-between text-base font-medium text-[#252525] py-2 border-b border-zinc-100"
              >
                <span>{link.name}</span>
